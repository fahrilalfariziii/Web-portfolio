  ...
  return (
    <section id="projects" className="projects">
      ...
      <div className="filter-buttons">
        ...
        <button className={`filter-btn ${filter === "ML" ? "active" : ""}`} onClick={() => setFilter("ML")}>ML</button>
        <button className={`filter-btn ${filter === "Web" ? "active" : ""}`} onClick={() => setFilter("Web")}>Web</button>
        <button className={`filter-btn ${filter === "CV" ? "active" : ""}`} onClick={() => setFilter("CV")}>CV</button>
        <button className={`filter-btn ${filter === "NLP" ? "active" : ""}`} onClick={() => setFilter("NLP")}>NLP</button>
        <button className={`filter-btn ${filter === "n8n" ? "active" : ""}`} onClick={() => setFilter("n8n")}>n8n</button>
      </div>
            ...
            <div className="project-footer">
              ...
              <div className="project-links">
                {project.webURL && isSafeUrl(project.webURL) && (
                  <a href={project.webURL} className="repo-link" target="_blank" rel="noopener noreferrer">
                    <span>View Site</span>
                  </a>
                )}
                {project.repoUrl && isSafeUrl(project.repoUrl) && (
                  <a href={project.repoUrl} className="repo-link" target="_blank" rel="noopener noreferrer">
      ...
      {shouldShowMoreButton && !showAll && (
        <div className="show-more-container">
          {filter === "all" ? (
            <a href="https://github.com/fahrilalfariziii" className="show-more-btn" target="_blank" rel="noopener noreferrer">Show More</a>
          ) : (
